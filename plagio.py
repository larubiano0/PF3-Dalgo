# Dynamic Programming function for counting the number of ways to reach a number m with divisible k steps
def count_divisible_paths(m, k):
    ...
    while k_multiples+k <= m: # It is not possible to reach a number greater than m 
        ...
        for i in range(k_multiples, m+1-k): # loop over all paths to i
            ...
            ans[i+k-1] = (ans[i+k-1]+aux_dp[i+k])%998244353 # add paths to i+k-1
        dp = aux_dp # update dp
